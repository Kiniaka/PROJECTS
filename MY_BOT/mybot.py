...
def show_phone_number():
    ...
    lista = []
    lista = [item['phone'] for item in data if ((show in item.get('name')) or (show in item.get('phone')))]
    if data == [] and lista == []:
...
def find_contact_by_letter_or_digit():
    ...
    lista = []
    lista = [item for item in data if ((contact_date in item.get('name')) or (contact_date in item.get('phone')))]
    if lista == []:
